...
void loop() {
  ...
  // ERROR
  error = SetPoint - ADCLuzFiltro;      // expresado en ADC
  float errorp = (error/SetPoint)*100;  // expresado en %
  error_acumulado += error;               //para el integral
  if (error_acumulado > 1023) error_acumulado = 1023;  // limites de acumulacion
  if (error_acumulado < -1023) error_acumulado = -1023;
  
  // SEÑAL DE CONTROL
  float s_control = -((kp * error) + (ki*error_acumulado)); 
  ...
  Serial.print(errorp);
  Serial.print("%");
  Serial.print("    - Error_acumulado(ADC): ");
  Serial.print(error_acumulado);
  Serial.print("    - KP: ");
  Serial.print(kp);
  Serial.print("    - KI: ");
  Serial.println(ki);
